...
async def procesar_combinada(message: Message, state: FSMContext):
    ...
    try:
        model = genai.GenerativeModel(model_name="gemini-1.5-flash-latest", system_instruction=SYSTEM_INSTRUCTION_COMBINADA)
...
async def process_quota_chat(message: Message, state: FSMContext):
    ...
    try:
        model = genai.GenerativeModel(model_name="gemini-1.5-flash-latest", system_instruction=SYSTEM_INSTRUCTION_CUOTA)
        prompt = (
